...
renderImage image = fromMaybe default $ map showImage image
    ...
    showImage = (\img ->
        H.img [H.src img, H.class' "storyImage"] [])
...
renderText s = H.div [] paragraphs
    where
    paragraphs = map (\txt ->
        H.p [] [H.text txt]) (split splitPattern s)
...
renderLink story link = H.a attrs [H.text link.text]
    where
    attrs = case (validateLink story link) of
        true -> [H.onClick $ ChangeScreen link.key, H.class' "activeStoryLink"
                ] <> fixedAttrs
        false -> [] <> fixedAttrs
